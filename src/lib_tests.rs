...
async fn disk_backed_spilled_value_corruption_fails_closed_for_reads_and_commit_guards() {
    ...
    assert!(matches!(err, QueryError::InternalError(message) if message.contains("persistent value hash mismatch")));
    ...
    let preflight = db
        ...
        .await;
    assert!(matches!(preflight, crate::preflight::PreflightResult::Err { reason } if reason.contains("persistent value hash mismatch")));
...
async fn disk_backed_kv_segment_corruption_fails_closed_for_point_prefix_and_secure_reads() {
    ...
    assert!(matches!(point_err, QueryError::InternalError(message) if message.contains("KV segment")));

    let prefix_err = db
        .kv_scan_prefix_no_auth("p", "app", b"seg:", 10, ConsistencyMode::AtLatest)
        .await
        .expect_err("prefix scan must fail on corrupt segment");
    assert!(matches!(prefix_err, QueryError::InternalError(message) if message.contains("KV segment")));
    ...
    assert!(matches!(secure_err, QueryError::InternalError(message) if message.contains("KV segment")));
}
