...
async fn non_pk_foreign_key_fails_closed_when_runtime_unique_index_missing() {
    ...
    exec.submit(Mutation::Upsert {
        ...
        row: Row {
            values: vec![Value::Text("p1".into()), Value::Text("p@example.com".into())],
        },
    })
    .await
    .expect("parent");
    ...
    let err = exec
        .submit(Mutation::Upsert {
            ...
            row: Row {
                values: vec![Value::Text("c1".into()), Value::Text("p@example.com".into())],
            },
        })
        .await
        .expect_err("missing runtime FK index must fail closed");
    assert!(matches!(err, AedbError::IntegrityError { message } if message.contains("missing runtime unique index")));
}
