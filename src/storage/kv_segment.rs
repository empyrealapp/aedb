...
fn bloom_bits_for_key(key: &[u8]) -> [u64; KV_SEGMENT_BLOOM_HASHES as usize] {
    ...
    let first = u64::from_le_bytes(copy_hash_bytes::<8>(bytes, 0));
    let second = u64::from_le_bytes(copy_hash_bytes::<8>(bytes, 8)) | 1;
...
fn copy_hash_bytes<const N: usize>(bytes: &[u8; 32], offset: usize) -> [u8; N] {
    let mut out = [0u8; N];
